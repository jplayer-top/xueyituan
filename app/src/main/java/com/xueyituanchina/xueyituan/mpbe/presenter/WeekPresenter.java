...
public class WeekPresenter extends BasePresenter<WeekListActivity> {

    private final ActiveModel mModel;

    public WeekPresenter(WeekListActivity iView) {
        super(iView);
        mModel = new ActiveModel(XYTServer.class);
    }

    public void weekList() {
        mModel.weekList().subscribe(new NetCallBackObserver<NearbyActiveBean>(new GetImplTip(mIView.mActivity)) {
            @Override
            public void responseSuccess(NearbyActiveBean nearbyActiveBean) {
                if (nearbyActiveBean != null && nearbyActiveBean.list != null && nearbyActiveBean.list.size() > 0) {
                    mIView.weekList(nearbyActiveBean);
                } else {
                    mIView.showEmpty();
                }
            }
